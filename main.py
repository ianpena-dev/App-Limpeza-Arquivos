...
import cleaner
from cleaner import clean_folder, format_size, log_history
import scheduler
...
class App(ctk.CTk):
    ...
    def _setup_folder_management(self, parent):
        ...
        self.folder_card = ctk.CTkFrame(parent, fg_color="#1e1e1e", corner_radius=15)
    ...
    def _setup_automation(self, parent):
        ...
        self.schedule_card = ctk.CTkFrame(parent, fg_color="#1e1e1e", corner_radius=15)
        ...
        ctk.CTkRadioButton(options, text="Somente Manual", value="Manual", variable=self.sched_var).grid(row=0, column=0, padx=10, pady=5, sticky="w")
        ctk.CTkRadioButton(options, text="Ao Iniciar", value="Logon", variable=self.sched_var).grid(row=0, column=1, padx=10, pady=5, sticky="w")
        ctk.CTkRadioButton(options, text="Ao Desligar", value="Shutdown", variable=self.sched_var).grid(row=1, column=0, padx=10, pady=5, sticky="w")

        interval_box = ctk.CTkFrame(options, fg_color="transparent")
        interval_box.grid(row=1, column=1, padx=10, pady=5, sticky="w")
        
        ctk.CTkRadioButton(interval_box, text="Intervalo (min):", value="Interval", variable=self.sched_var).pack(side="left")
        ...
        self.apply_button = ctk.CTkButton(
            ...
            width=250, height=40, fg_color="#0078d7", hover_color="#005a9e",
    ...
    def _setup_history_tab(self, parent):
        ...
        ctk.CTkLabel(header, text="Histórico de Atividades", font=ctk.CTkFont(size=16, weight="bold")).pack(side="left", padx=10)
        
        self.clear_history_btn = ctk.CTkButton(
            header, text="Limpar Histórico", width=120, height=28, 
            fg_color="#d35b5b", hover_color="#b24a4a",
    ...
    def refresh_history(self):
        ...
        if not os.path.exists(HISTORY_FILE):
            ctk.CTkLabel(self.history_frame, text="Nenhum histórico disponível.", text_color="#5a5a5a").pack(pady=40)
            return
        ...
        try:
            ...
            if not history:
                ctk.CTkLabel(self.history_frame, text="O histórico está vazio.", text_color="#5a5a5a").pack(pady=40)
                return
                
            for entry in history:
                card = ctk.CTkFrame(self.history_frame, fg_color="#1e1e1e", corner_radius=10)
                card.pack(fill="x", padx=5, pady=5)
                
                # Layout mais bonito para o histórico
                date_label = ctk.CTkLabel(card, text=f"📅 {entry['date']}", font=ctk.CTkFont(size=12, weight="bold"), text_color="#aaaaaa")
                date_label.pack(side="left", padx=15, pady=10)
                
                info_text = f"Items: {entry['items']}  |  Recuperado: {entry['size']}"
                info_label = ctk.CTkLabel(card, text=info_text, font=ctk.CTkFont(size=12, weight="bold"), text_color="#00d778")
